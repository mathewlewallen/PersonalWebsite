...
import LazyImage from '@ml/components/lazy-image';
import { AiOutlineContainer } from 'react-icons/ai';
import { getDevPost, getMediumPost } from '@arifszn/blog-js';
import { formatDistance } from 'date-fns';
import { SanitizedBlog, Article } from '@ml/types';
import { ga, skeleton } from '@ml/lib/utils';
...
const BlogCard = ({
  ...
  const { source, username, limit } = blog;

  useEffect(() => {
    if (source === 'medium') {
      getMediumPost({ user: username }).then(setArticles);
    } else if (source === 'dev') {
      getDevPost({ user: username }).then(setArticles);
    }
  }, [source, username]);
      ...
      articles.slice(0, limit).map((article) => (
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            try {
              if (googleAnalyticsId && typeof ga?.event === 'function') {
